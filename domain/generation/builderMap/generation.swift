...
public struct LevelBuilder {
    static public func buildLevel(roomGenerator: RoomGeneratorProtocol = RoomGenerator(), corridorsGenerator: CorridorsGeneratorProtocol = CorridorsGenerator()) -> Level {
        ...
        return Level(rooms, corridors, self.setRandomStartRoomAndExit(rooms, graph))
    }

    static private func setRandomStartRoomAndExit(_ rooms: [Room], _ graph: Graph) -> Position {
        ...
        return self.generateExit(rooms[indexEnd])
    }

    static private func generateExit(_ room: Room) -> Position {
